...
Thread::Thread() {
  ...
  if(TraceObjects){
      ...
      if(TraceObjectsPointers){
          ...
          _event_root_ptrs = new EventPointers();
      }
  }
}
...
Thread::~Thread() { 
  if(TraceObjects){
      ...
      if(TraceObjectsPointers){
          ...
          delete _event_root_ptrs;
          _event_root_ptrs = NULL;
      }
  }
...
void Thread::add_event_root_ptr(oop p) {
    _event_root_ptrs->add(p);
}
...
void Threads::possibly_parallel_oops_do(OopClosure* f, CLDClosure* cld_f, CodeBlobClosure* cf) {
  ...
  ALL_JAVA_THREADS(p) {
    if (p->claim_oops_do(is_par, cp)) {
      if(TraceObjectsPointers) { Thread::current()->get_event_root_ptrs()->set_meta(EVENTS_GC_ROOT_PTR, (uintptr_t) p, true);}
      p->oops_do(f, cld_f, cf);
      if(TraceObjectsPointers){ Thread::current()->get_event_root_ptrs()->flush(); } 
    }
  }
  VMThread* vmt = VMThread::vm_thread();
  if (vmt->claim_oops_do(is_par, cp)) {
    if(TraceObjectsPointers) { Thread::current()->get_event_root_ptrs()->set_meta(EVENTS_GC_ROOT_PTR, (uintptr_t) vmt, true);}
