...
void DirtyCardToOopClosure::walk_mem_region(MemRegion mr,
  ...
  for (; bottom < top; bottom += _sp->block_size(bottom)) {
    ...
    if (_sp->block_is_obj(bottom) &&
      ...
      if(TraceObjectsPointers) { Thread::current()->get_event_obj_ptrs()->set_meta(EVENTS_GC_CROSSGENERATIONAL_PTR, (uintptr_t) (HeapWord*) bottom, true); }
...
void ContiguousSpace::allocate_temporary_filler(int factor) {
  ...
  if (size >= (size_t)align_object_size(array_header_size)) {
    ...
    if(TraceObjectsAllocations) {
        ...
        if(allocation_site >= ALLOCATION_SITE_IDENTIFIER_FIRST_CUSTOM) {
            allocation_site = ALLOCATION_SITE_IDENTIFIER_VM_INTERNAL;
        }
        if(TraceObjectsSaveAllocationSites) {
