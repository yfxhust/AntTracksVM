...
void EventsRuntime::fire_mirror_klass_write(oop java_class, Klass* k) {
    AllocatedTypes::write_mirror_symbols(java_class, k);
}
...
void EventsRuntime::fire_gc_start_end(EventType event_type, jint id, GCType gc_type, GCCause::Cause gc_cause, bool failed, bool gc_only){
#ifdef ASSERT
    ...
    fire_event_sync(Thread::current(), event_type, 6, local_buffer_top, Sync_Full, (EVENTS_GC_END == event_type && gc_only)) {
        rel_addr_base = compute_rel_addr_base();
        write_gc_event(local_buffer_top++, event_type, gc_type, gc_cause, failed);
...
void EventsRuntime::fire_gc_move_fast(oop from_addr, SpaceType to_space){
    ...
    if(TraceObjectsGCOptimization && get_rel_addr((HeapWord*) from_addr, &rel_from_addr, 32 - 8 - 2)) {
...
void EventsRuntime::fire_gc_root_ptr(EventType event_type, uintptr_t obj, bool obj_is_inside_heap) {
#ifdef ASSERT
    assert(!EventsRuntime::meta_events_only, "only meta_events allowed at file start");
#endif
    Thread* thread = Thread::current();
    
    EventPointerList* ptr_list = thread->get_event_root_ptrs()->get();
    jint event_size = 3 + ptr_list->get_words();
    fire_event(thread, event_type, event_size, local_buffer_top) {
         write_gc_ptr_event(local_buffer_top++, event_type, NO_TO_SPACE, ptr_list->get_kinds());
         
         if(!obj_is_inside_heap){
             write_wide_word(local_buffer_top++, (jlong) obj); local_buffer_top++;
         } else {
             write_abs_addr(local_buffer_top++, (HeapWord*) obj); local_buffer_top++;   
         }
    //     write_length(local_buffer_top++, ptr_list->get_size());
         fire_gc_ptr(&local_buffer_top, ptr_list);
    }
...
void EventsRuntime::fire_thread_alive(Thread* thread) {
    ...
    fire_event(Thread::current(), EVENTS_THREAD_ALIVE, 3 + (name_length / sizeof(jint)) + ((name_length % sizeof(jint) != 0) ? (1) : (0)), local_buffer_top) {
        ...
        write_abs_addr(local_buffer_top, (HeapWord*) thread);
        local_buffer_top += 2;
        jint word = 0;
        int filled = 0;
...
inline void EventsRuntime::write_gc_event(jint* local_buffer_top, EventType event_type, GCType gc_type, GCCause::Cause gc_cause, bool failed){
