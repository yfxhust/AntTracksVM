...
jint AllocationTracingIO::header_size = 0;
jint* AllocationTracingIO::header = NULL;
...
void AllocationTracingIO::init_header() {
    critical_if(header_size == 0) {
        header_size = 42;
        header = (jint*) calloc(sizeof(jint), header_size);
...
void AllocationTracingIO::write_global_header() {
    ...
    write(&header_size, sizeof(jint));
    write(header, sizeof(jint) * header_size);
