...
AllocatedTypeIdentifier AllocatedTypes::to_allocated_type(Klass* allocated_type){
    if(allocated_type != NULL){
        synchronized_if(lock, allocated_type->get_allocated_type_identifier() == ALLOCATED_TYPE_IDENTIFIER_UNKNOWN || allocated_type->oop_is_instanceMirror()) {
            ...
            allocated_type->set_allocated_type_identifier(allocated_type_id); // overwrite id but not write symbols
            if(!allocated_type->oop_is_instanceMirror()) write_symbols(allocated_type_id, allocated_type); // instance mirrors are written later on
            self_monitoring(1) {
                AllocationTracingSelfMonitoring::report_new_allocated_type();
            }
...
void AllocatedTypes::write_mirror_symbols(oop o, Klass* k){
    synchronized(lock) {
        assert(o->is_instanceMirror(), "What kind of oop is this?");
        Klass* mirror = o->klass();
        AllocatedTypeIdentifier id = mirror->get_allocated_type_identifier();
        write_symbols(id, mirror, o, k);
    }
}
...
void AllocatedTypes::write_symbols(AllocatedTypeIdentifier allocated_type_id, Klass* allocated_type, oop o, Klass* k){
    ...
    symbols_writer->write_symbols(symbol, o, k);
}
