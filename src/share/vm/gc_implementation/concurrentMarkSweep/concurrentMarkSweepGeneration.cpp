  ...
  _did_compact(false)
{
  ...
  if (TraceObjectsGC) {
    EventsRuntime::fire_space_creation(CMS_GC_OLD_ID, _cmsSpace->bottom(), _cmsSpace->end());
    EventsRuntime::fire_space_alloc(CMS_GC_OLD_ID, SPACE_MODE_NORMAL, OLD_SPACE);
  }
}
...
void ConcurrentMarkSweepGeneration::compute_new_size() {
  ...
  if (incremental_collection_failed()) {
    ...
    bool grow_successful = grow_to_reserved();
    if (grow_successful && TraceObjectsGC) {
        PostponeSpaceRedefineBlock postponeredefine = PostponeSpaceRedefineBlock(1);
        assert(_cmsSpace->bottom() < _cmsSpace->end(), "sanity check");
        EventsGCRuntime::schedule_space_redefine(CMS_GC_OLD_ID, _cmsSpace->bottom(), _cmsSpace->end(), true);
    }
    return;
  }
  ...
  if (res != NULL) {
    ...
    if (TraceObjectsGC) {
      ...
      if(TraceObjectsSaveAllocationSites) {
          AllocationSiteStorage::store(thread, oop(res), allocation_site);
      }
      EventsRuntime::fire_obj_alloc_slow(thread, oop(res), allocation_site, EVENTS_ALLOC_SLOW, adjustedSize);
    }
  }
  return res;
}
...
oop ConcurrentMarkSweepGeneration::promote(oop obj, size_t obj_size) {
  ...
  if (res != NULL) {
    ...
    if (TraceObjectsGC) {
      EventsGCRuntime::fire_gc_move_slow(obj, res, (int) (CompactibleFreeListSpace::adjustObjectSize(obj->size())));
    }
  }
  return res;
}
...
NOT_PRODUCT(
  ...
  if (should_compact) {
    ...
    if (TraceObjectsGC) {
      if (first_state > Idling
              && first_state < Sweeping) {
        /* blunt lie, as we want the parser to compact from the current state,
         * and not from the interim it is currently in 
         * i.e. promoted objects are not yet commited and can't be found by the parser */
        EventsGCRuntime::fire_gc_failed(true);
        EventsGCRuntime::fire_gc_end(GCType_Minor, _gc_id, GCCause::_cms_final_remark);
      }
      assert(!EventsGCRuntime::is_gc_active(), "not possible/state invariant");
      _gc_id = EventsGCRuntime::fire_gc_start(GCType_Minor, GCCause::_no_gc);
      /* compact all spaces */
      EventsGCRuntime::fire_gc_info(true, _gc_id);
      EventsGCRuntime::fire_gc_info(false, _gc_id);
    }
...
bool ConcurrentMarkSweepGeneration::expand(size_t bytes, size_t expand_bytes) {
  bool success = CardGeneration::expand(bytes, expand_bytes);
  if (TraceObjectsGC && success) { // immediately as it should satisfy a allocation
      assert(_cmsSpace->bottom() < _cmsSpace->end(), "sanity check");
      EventsRuntime::fire_space_redefine(CMS_GC_OLD_ID, _cmsSpace->bottom(), _cmsSpace->end());
  }
  return success;
}
...
void CMSCollector::checkpointRootsInitialWork(bool asynch) {
  ...
  if (TraceObjectsGC) {
    _gc_id = EventsGCRuntime::fire_gc_start(GCType_Minor, gch->gc_cause());
  }
...
void CMSCollector::checkpointRootsFinalWork(bool asynch,
  bool clear_all_soft_refs, bool init_mark_was_synchronous) {
  if (TraceObjectsGC) {
    // force a merge of the old space and promoted objects
    EventsGCRuntime::fire_gc_failed(true);
    EventsGCRuntime::fire_gc_end(GCType_Minor, _gc_id, GCCause::_cms_initial_mark, true);
  }
  ...
  if (TraceObjectsGC) {
    // and start again.
    _gc_id = EventsGCRuntime::fire_gc_start(GCType_Minor, GCCause::_cms_final_remark);
    EventsGCRuntime::fire_gc_info(true, _gc_id);
  }
}
...
void CMSCollector::sweep(bool asynch) {
  ...
  if (asynch) {
    ...
    // First sweep the old gen
    {
      ...
      if (TraceObjectsGC) {
        // fire here, whilst holding the locks, to have a more accurate GC_end
        EventsGCRuntime::fire_gc_end(GCType_Minor, _gc_id, GCCause::_cms_final_remark, false, true);
      }
    }
    ...
    // already have needed locks
    {
      sweepWork(_cmsGen,  asynch);
      if (TraceObjectsGC) {
        // don't have to aquire CMStoken, as we do this as a foreground collection
        EventsGCRuntime::fire_gc_end(GCType_Minor, _gc_id, GCCause::_cms_final_remark, false, true);
      }
    }
