...
CompactibleFreeListSpace::refillLinearAllocBlock(LinearAllocBlock* blk) {
  ...
  if (fc != NULL) {
    ...
    if (TraceObjectsGC) { // allocations during prologue (those are freed at the end of the current cycle)
        EventsGCRuntime::fire_gc_filler_alloc((oop) fc);
    }
  }
}
...
HeapWord* CFLS_LAB::alloc(size_t word_sz) {
  ...
  if (TraceObjectsGC && UseConcMarkSweepGC && res != NULL) {
      assert(word_sz > 0, "has to be");
      EventsGCRuntime::fire_plab_alloc((HeapWord*)res, word_sz * HeapWordSize);
  }
  return (HeapWord*)res;
}
