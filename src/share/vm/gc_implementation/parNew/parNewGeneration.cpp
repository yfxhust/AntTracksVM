  ...
  _plab_stats(YoungPLABSize, PLABWeight)
{
  ...
  if (TraceObjectsGC) {
    EventsRuntime::fire_space_creation(PARALLEL_GC_EDEN_ID,       eden()->bottom(), eden()->end());
    EventsRuntime::fire_space_creation(PARALLEL_GC_SURVIVOR_1_ID, from()->bottom(), from()->end());
    EventsRuntime::fire_space_creation(PARALLEL_GC_SURVIVOR_2_ID,   to()->bottom(),   to()->end());
...
void ParNewGeneration::collect(bool   full,
  ...
  jint gc_id;
  if (TraceObjectsGC) {
      gc_id = EventsGCRuntime::fire_gc_start(GCType_Minor, gch->gc_cause());
      EventsGCRuntime::fire_gc_info(false, gc_id);
  }
