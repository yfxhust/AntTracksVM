...
void ASParNewGeneration::resize_spaces(size_t requested_eden_size,
  ...
  if (TraceObjectsGC) {
      EventsRuntime::fire_space_redefine(PARALLEL_GC_EDEN_ID,       eden()->bottom(), eden()->end());
      EventsRuntime::fire_space_redefine(PARALLEL_GC_SURVIVOR_1_ID, from()->bottom(), from()->end());
      EventsRuntime::fire_space_redefine(PARALLEL_GC_SURVIVOR_2_ID,   to()->bottom(),   to()->end());
  }
