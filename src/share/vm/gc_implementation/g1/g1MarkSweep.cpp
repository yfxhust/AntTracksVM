  ...
  bool doHeapRegion(HeapRegion* hr) {
    if (hr->isHumongous()) {
      if (hr->startsHumongous()) {
        ...
        if (obj->is_gc_marked()) {
          if(TraceObjectsPointers) Thread::current()->get_event_obj_ptrs()->set_meta(EVENTS_GC_KEEP_ALIVE, (uintptr_t)(hr->bottom()), true, 0, -1, false);
          obj->init_mark();
...
void G1MarkSweep::mark_sweep_phase4() {
  if(TraceObjectsGC) {
      EventsGCRuntime::fire_gc_info(true, gc_id);
  }
