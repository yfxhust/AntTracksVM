...
bool G1CollectedHeap::do_collection(bool explicit_gc,
  ...
  int id;
  if(TraceObjects) {
      id = EventsGCRuntime::fire_gc_start(GCType_Major, gc_cause());
  ...
  
  {
    ...
    if(TraceObjects) {
        EventsGCRuntime::fire_gc_end(GCType_Major, id, gc_cause());
    }
    ...
    if (!oopDesc::is_null(heap_oop)) {
      ...
      if (_g1h->is_obj_dead_cond(obj, _vo)) {
        ...
        if(TraceObjectsPointers){
            Thread::current()->add_event_root_ptr(obj);
        }
      }*/
    }
    /*else if(TraceObjectsPointers){
       if(TraceObjectsPointers){
            Thread::current()->add_event_root_ptr(NULL);
        } 
    }*/
  }
...
G1CollectedHeap::do_collection_pause_at_safepoint(double target_pause_time_ms) {
  ...
  int id;
  if(TraceObjects) {
      id = EventsGCRuntime::fire_gc_start(GCType_Minor, gc_cause());
      G1MarkSweep::gc_id = id;
  }
  ...
  // Inner scope for scope based logging, timers, and stats collection
  {
    ...
    { // Call to jvmpi::post_class_unload_events must occur outside of active GC
...

      {
        ...
        if(TraceObjectsGC) {
            EventsGCRuntime::fire_gc_info(false, id);
        }
        
        // Actually do the work...
        evacuate_collection_set(evacuation_info);
        
        if(TraceObjectsGC && _evacuation_failed) {
            EventsGCRuntime::fire_gc_failed(false);
        }
  ...
  if(TraceObjects) {
      EventsGCRuntime::fire_gc_end(GCType_Minor, id, gc_cause());
  }
