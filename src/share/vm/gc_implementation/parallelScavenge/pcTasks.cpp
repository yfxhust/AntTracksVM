...
void ThreadRootsMarkingTask::do_it(GCTaskManager* manager, uint which) {
  ...
  if (_java_thread != NULL) {
    if(TraceObjectsPointers) { Thread::current()->get_event_root_ptrs()->set_meta(EVENTS_GC_ROOT_PTR, (uintptr_t) _java_thread, true); }
  ...
  if (_vm_thread != NULL){
    if(TraceObjectsPointers) { Thread::current()->get_event_root_ptrs()->set_meta(EVENTS_GC_ROOT_PTR, (uintptr_t) _vm_thread, true); }
    _vm_thread->oops_do(
        &mark_and_push_closure,
        &mark_and_push_from_clds,
        &mark_and_push_in_blobs);
...
void MarkFromRootsTask::do_it(GCTaskManager* manager, uint which) {
  ...
  if(TraceObjectsPointers){ Thread::current()->get_event_root_ptrs()->set_meta(EVENTS_GC_ROOT_PTR, _root_type, false); }
  switch (_root_type) {
    case universe:
