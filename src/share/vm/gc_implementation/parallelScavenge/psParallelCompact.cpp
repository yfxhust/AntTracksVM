...
//       ((CardTableModRefBS*) Universe::heap()->barrier_set())->printCardTable(true);
//       
        id = EventsGCRuntime::fire_gc_start(GCType_Major, heap->gc_cause());
        EventsGCRuntime::fire_gc_info(true, id);
    ...
    if (UseAdaptiveSizePolicy) postpone_space_redefine {
...
void PSParallelCompact::adjust_roots() {
  ...
  EventPointers* ptrs = Thread::current()->get_event_root_ptrs();
  if(TraceObjectsPointers){ ptrs->set_meta(EVENTS_GC_ROOT_PTR, MarkFromRootsTask::universe, false); }
  Universe::oops_do(adjust_pointer_closure());
  if(TraceObjectsPointers){ ptrs->flush(); ptrs->set_meta(EVENTS_GC_ROOT_PTR, MarkFromRootsTask::jni_handles, false); }
  JNIHandles::oops_do(adjust_pointer_closure());   // Global (strong) JNI handles
  if(TraceObjectsPointers){ ptrs->flush(); ptrs->set_meta(EVENTS_GC_ROOT_PTR, MarkFromRootsTask::threads, false); }
  CLDToOopClosure adjust_from_cld(adjust_pointer_closure());
  Threads::oops_do(adjust_pointer_closure(), &adjust_from_cld, NULL);
  if(TraceObjectsPointers){ ptrs->flush(); ptrs->set_meta(EVENTS_GC_ROOT_PTR, MarkFromRootsTask::object_synchronizer, false); }
  ObjectSynchronizer::oops_do(adjust_pointer_closure());
  if(TraceObjectsPointers){ ptrs->flush(); ptrs->set_meta(EVENTS_GC_ROOT_PTR, MarkFromRootsTask::flat_profiler, false); }
  FlatProfiler::oops_do(adjust_pointer_closure());
  if(TraceObjectsPointers){ ptrs->flush(); ptrs->set_meta(EVENTS_GC_ROOT_PTR, MarkFromRootsTask::management, false); }
  Management::oops_do(adjust_pointer_closure());
  if(TraceObjectsPointers){ ptrs->flush(); ptrs->set_meta(EVENTS_GC_ROOT_PTR, MarkFromRootsTask::jvmti, false); }
  JvmtiExport::oops_do(adjust_pointer_closure());
  if(TraceObjectsPointers){ ptrs->flush(); ptrs->set_meta(EVENTS_GC_ROOT_PTR, MarkFromRootsTask::system_dictionary, false); }
  SystemDictionary::oops_do(adjust_pointer_closure());
  if(TraceObjectsPointers){ ptrs->flush(); ptrs->set_meta(EVENTS_GC_ROOT_PTR, MarkFromRootsTask::class_loader_data, false); }
  ClassLoaderDataGraph::oops_do(adjust_pointer_closure(), adjust_klass_closure(), true);
  if(TraceObjectsPointers){ ptrs->flush(); ptrs->set_meta(EVENTS_GC_ROOT_PTR, 11, false /* weak roots */); }
  ...
  if(TraceObjectsPointers){ ptrs->flush(); }
}
